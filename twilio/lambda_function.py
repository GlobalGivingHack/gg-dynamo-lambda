...
def get_user_details(from_number):
    ...
    if response_dynamo['Count'] != 1:
        return None
    return (response_dynamo['Items'][0]['name'], response_dynamo['Items'][0]['charity'])

def save_message(message_id, user, charity, message, media_url):
    table_messages.put_item(Item={'message_id': message_id, 'user_id': user, 'charity_id': charity, 'message': message, 'media_url': media_url})
...
def fire_sns(charity, message):
    message = {'charity_id': charity, 'message': message}
...
def lambda_handler(event, context):
    ...
    user, charity = get_user_details(from_number)
    if user == None:
    ...
    save_message(message_id, user, charity, message, media_url)
    fire_sns(charity, message)
